...
export default function Order() {
  ...
  const renderBreakfastItems = () => {
    if (breakfastButton === true) {
      return (
        <React.Fragment>
          {firstMenu.breakfast.map((object) => (
            <MenuItem
  ...
  const renderItemPopUp = () => {
    if (expandItemButton === true) {
      return (
        <ExpandMenuItem
          children={
            <div className="close-this-item" onClick={() => setMenuItem(false)}>
              X
            </div>
          }
          itemName={itemName}
          ...
          addToCart={
            <div id="add-to-cart-close" onClick={() => setMenuItem(false)}>
              <h2>Add to Cart</h2>
            </div>
          }
        />
...
const ExpandMenuItem: React.FC<ExpandProps> = ({
  itemName,
  itemDescription,
  itemPrice,
  itemPicture,
  ...
  return (
    <div className="ExpandMenuItem">
      ...
      <div className="item-cart">
        ...
        <div className="plus-butt" onClick={() => setCounter(orderCounter + 1)}>
          +
        </div>
      </div>
      <div
